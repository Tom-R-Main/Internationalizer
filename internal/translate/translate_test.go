...
func TestRunTranslatesFluentUnitsWithDeveloperContext(t *testing.T) {
	...
	context := ""
	for _, entry := range provider.request.Entries {
		if entry.Key == "open-button" {
			context = entry.Context
		}
	}
	if len(provider.request.Entries) != 3 || context != "# A command, not a noun." {
		t.Fatalf("provider entries = %#v", provider.request.Entries)
	}
	if !strings.Contains(provider.request.SystemPrompt, "Fluent variables") {
		t.Fatalf("provider did not receive Fluent prompt: %s", provider.request.SystemPrompt)
	}
